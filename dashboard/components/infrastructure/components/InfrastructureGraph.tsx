...
function nodeVertices(vertices: Vertices[], edges: Edges[]): {withEdges: Node[], withoutEdges: Node[]} {
    ...
    const withoutEdges: Node[] = new Array<Node>();
...
function InfrastructureGraph({ vertices, edges}: InfrastrucuteGraphProps) {
    ...
    const [state, setState] =useState<{
        ...
        events: {
          ...
          click: (event: { }) => void;
          doubleClick: (event: { pointer: { canvas: { x: number; y: number } } }) => void;
        };
      }>({
        counter: 10,
        graph: {
